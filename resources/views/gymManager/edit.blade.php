...
<div class="content-wrapper pb-4">
    ...
    <section class="content">
        <form action="{{url('/admin/storeEditManager/'.$singleManager->id)}}" method="post"
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input autofocus required minlength="3" maxlength="50" type="text" id="name"
                                    value="{{$singleManager->name}}" class="form-control" value="" name="name">
                            </div>
                            <div class="form-group">
                                <label for="email">Email</label>
                                <input required type="email" id="email" class="form-control" name="email"
                                    value="{{$singleManager->email}}">
                            </div>
                            <div class="form-group">
                                ...
                                <select required class=" form-control" name="city_id" id="city">
                                    <optgroup label="Available City">
                                        ...
                                        <option value={{ $city->id }}
                                            <?php echo $city->id == $singleManager->city_id? 'selected': '';  ?>>
                                            >{{ $city->name }}</option>
                                        @endforeach
                                    </optgroup>
                                </select>
                            </div>
                            <input type="hidden" name="profile_image_saved" value="{{$singleManager->profile_image}}">
                            <div class="form-group">
